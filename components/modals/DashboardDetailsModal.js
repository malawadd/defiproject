...
export const DashboardDetailsModal = ({ show, onClose, item, token }) => {
  ...
  const {
    ...
    const startDate = new Date(pool?.startDate * 1000). toLocaleDateString("en-US");
    const startTime = new Date(pool?.startDate * 1000). toLocaleDateString("en-US");
    const endDate = new Date(pool?.endDate * 1000). toLocaleDateString("en-US");
    const endTime = new Date(pool?.endDate * 1000). toLocaleDateString("en-US");
    ...
    const modalContent = show ? (
        <div className={s.modalOverlay}>
            ...
            <div className={s.modalBody}>
                ...
                <div className={s.modalRow}>
                    <p className={s.label}>Total Amount in Taamin</p>
                    <p className={s.value}>
